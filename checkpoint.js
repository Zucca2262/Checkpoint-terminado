...
function joinLinkedList(linkedListOne, linkedListTwo) {
  ...
  linkedList.add(linkedListOne.head.value);
  linkedList.add(linkedListTwo.head.value);

  var node1 = linkedListOne.head.next
  var node2 = linkedListTwo.head.next
  while (node1 !== null) {
    linkedList.add(node1.value)
    linkedList.add(node2.value)
    node1=node1.next
    node2=node2.next
  }
  return linkedList
  ...
  return false
    ...
    return function isAllowed(array) {
      ...
      for (let i = 0; i < array.length; i++) {
        ...
        if (array[i].allowed.includes(country)) {
          newpersonas.push(array[i])
        }
...
function cardGame(playerOneCards, playerTwoCards) {
  ...
  while (playerOneCards.size() > 0  ){
    ...
    if(playerOneCards.peek().attack > playerTwoCards.peek().defense){
     castillo2 = castillo2 - (playerOneCards.peek().attack - playerTwoCards.peek().defense)
      }
    
  if(playerTwoCards.peek().attack > playerOneCards.peek().defense){
    castillo1 = castillo1 - (playerTwoCards.peek().attack - playerOneCards.peek().defense)
    } 
